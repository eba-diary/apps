...
namespace Sentry.data.Core.Factories.Fields
{
    public class DateFieldDtoFactory : FieldDtoFactory
    {
        ...
        public DateFieldDtoFactory(KeyValuePair<string, JsonSchemaProperty> prop, int rowPosition, bool array)
        {
            _property = prop;
            _array = array;
        ...
        public override BaseFieldDto GetField()
        {
            ...
            {
                return new DateFieldDto(_property, _rowPosition, _array);
            }
            else if (IsRow)
            {
