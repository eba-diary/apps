...
namespace Sentry.data.Core.Factories.Fields
{
    public class DecimalFieldDtoFactory : FieldDtoFactory
    {
        ...
        public DecimalFieldDtoFactory(KeyValuePair<string, JsonSchemaProperty> prop,int rowPosition, bool array)
        {
            _property = prop;
            _array = array;
        ...
        public override BaseFieldDto GetField()
        {
            ...
            {
                return new DecimalFieldDto(_property, _rowPosition, _array);
            }
            else if (IsRow)
            {
