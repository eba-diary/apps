...
namespace Sentry.data.Infrastructure
{
    public class EventService : IEventService
    {
        public void PublishSuccessEventByConfigId(string eventTypeDescription, string userId, string reason, int configId)
        {
            ...
            Task.Factory.StartNew(() => SaveEvent(eventTypeDescription, userId, reason, datasetId, configId, 0), TaskCreationOptions.RunContinuationsAsynchronously);
        }
        ...
        {
            Task.Factory.StartNew(() => SaveEvent(eventTypeDescription, userId, reason, 0, 0, 0), TaskCreationOptions.RunContinuationsAsynchronously);
        }
        ...
        {
            Task.Factory.StartNew(() => SaveEvent(eventTypeDescription, userId, reason, 0, 0, notificationId), TaskCreationOptions.RunContinuationsAsynchronously);
        }


        private void SaveEvent(string eventTypeDescription, string userId, string reason, int datasetId, int configId, int notificationId)
        {
            using (IDatasetContext _datasetContext = Bootstrapper.Container.GetNestedContainer().GetInstance<IDatasetContext>())
            {
                ...
                Event evt = CreateEvent(et, status, userId, reason, datasetId, configId, notificationId);
        ...
        private Event CreateEvent(EventType eventType, Status status, string userId, string reason, int? datasetId, int? configId, int? notificationId)
        {
            return new Event()
            {
                EventType = eventType,
