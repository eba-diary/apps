        ...
        public static void Run(string interval)
        {
            try
            {
                ...
                using (container = Sentry.data.Infrastructure.Bootstrapper.Container.GetNestedContainer())
                {
                    ...
                    {
                        case "Hourly":
                            events = _datasetContext.EventsSince(DateTime.Now.AddHours(-1), false);
                            break;

                        case "Daily":
                            events = _datasetContext.EventsSince(DateTime.Now.AddDays(-1), false);
                            break;

                        case "Weekly":
                            events = _datasetContext.EventsSince(DateTime.Now.AddDays(-7), false);
                            break;
                    ...
                    foreach (Event _event in events)
                    {
...
#else
                        //STEP 1:  CREATE UserEvents for UserWhoStarted (Author)
                        int n;
                        ...
                        //STEP 2:   Create UserEvents for people that subscribed to the _event generated by the UserWhoStarted (Author)
                        foreach (DatasetSubscription ds in subsThatMatch)
                        {
                         ...
                         if (ds.SentryOwnerName == "082698" || ds.SentryOwnerName == "072984" || ds.SentryOwnerName == "072186")
                            ...
                            //ONLY create new UserEvent here if subscription owner <> UserWhoStarted AKA author (otherwise you would have double the UserEvents)
