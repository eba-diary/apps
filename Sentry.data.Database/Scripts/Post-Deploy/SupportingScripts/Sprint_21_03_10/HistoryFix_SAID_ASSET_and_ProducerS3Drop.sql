...
*	1.1		1/14	Split Raw Storage and S3 Drop updates, adjusted @OldDropPrefix logic
...
DECLARE @NewSourceBucket VARCHAR(40) = (select TargetStorageBucket from DataAction where ActionType = 'ProducerS3Drop')
DECLARE @NewProducerS3DropActionId INT = (Select Id from DataAction where Name = 'Producer S3 Drop')
DECLARE @OldDropPrefix VARCHAR(25)

Select @ENV

if (@ENV = 'PROD' or @ENV = 'QUAL')
BEGIN
	SET @OldDropPrefix = 'droplocation/data/'
END
else if (@ENV = 'NRTEST' or @ENV = 'TEST')
BEGIN
	SET @OldDropPrefix = 'droplocation/data-test/'
END
else if (@ENV = 'NRDEV' or @ENV = 'DEV')
BEGIN
	SET @OldDropPrefix = 'droplocation/data-dev/'
END
else
BEGIN
	SET @OldDropPrefix = 'droplocation/data-' + LOWER(@ENV) + '/'
END

select 'OldDropPrefix: ' + @OldDropPrefix
/**********************************
...
if (@ENV = 'TEST' or @ENV = 'NRTEST' or @ENV = 'DEV' or @ENV = 'NRDEV')
BEGIN
...
select 
	...
	DFS.Action_Id,
	@NewProducerS3DropActionId as 'NewActionId',
	DFS.DataAction_Type_Id,
	12 as 'NewDataAction_Type_Id',
	DFS.TriggerKey,
	REPLACE(DFS.TriggerKey, @OldDropPrefix, 'droplocation/data/' + EF.SaidAsset + '/') as 'NewTriggerKey',
	DFS.TargetPrefix,
...
select 
	...
	EF.SaidAsset,
	DFS.SourceDependencyBucket,
	@NewSourceBucket as 'NewSourceDependencyBucket',
	DFS.SourceDependencyPrefix,
	REPLACE(DFS.SourceDependencyPrefix, @OldDropPrefix, 'droplocation/data/' + EF.SaidAsset + '/') as 'NewSourceDependencyPrefix',	
